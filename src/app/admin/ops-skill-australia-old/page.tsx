...
interface SkillAustraliaOldOps {
  id: number
...
export default function OpsSkillAustraliaOldPage() {
  const [operations, setOperations] = useState<SkillAustraliaOldOps[]>([])
  ...
  const fetchOperations = async () => {
    setLoading(true)
    try {
      const databaseOperations = (await searchOperationCases({ module: 'skill-australia', search: searchTerm || null, limit: 100 }))
        .map(mapOperationsRowToListItem) as unknown as SkillAustraliaOldOps[]
      // Apply filters
  ...
  const exportReport = () => {
    alert('Exporting Australia Old Skill Assessment Operations Report')
  }
    ...
    <MainLayout>
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Australia Old Skill Assessment</h1>
            <p className="text-gray-600">Manage archived and legacy Australian skilled migration applications</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center">
                <Flag className="h-8 w-8 text-purple-600 mr-3" />
                <div>
                  <p className="text-sm text-gray-500">Average Points</p>
                  <p className="text-2xl font-bold text-gray-900">
                    {operations.length > 0 ? Math.round(operations.reduce((sum, op) => sum + op.pointsScore, 0) / operations.length) : 0}
                  </p>
                </div>
              </div>
            </CardContent>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">{operation.occupation} (ANZSCO {operation.anzscoCode})</p>
                    </div>
                  </div>
                  <div className="flex items-center space-x-2">
                    <Badge className="bg-green-100 text-green-800">{operation.targetCountry}</Badge>
                    {getScoreBadge(operation.pointsScore)}
              ...
              <CardContent className="pb-4">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center justify-between">
                        <span className="text-sm text-gray-600">Points Score:</span>
                        {getScoreBadge(operation.pointsScore)}
                      </div>
                      <div className="flex items-center justify-between">
                ...
                <div className="border-t pt-4">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                    ...
                    <div className="text-center">
                      <div className="text-2xl font-bold text-pink-600">{operation.pointsBreakdown.australianStudy}</div>
                      <div className="text-xs text-gray-500">AU Study</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-indigo-600">{operation.pointsBreakdown.partnerSkills}</div>
                      <div className="text-xs text-gray-500">Partner</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-teal-600">{operation.pointsBreakdown.regionalStudy}</div>
                      <div className="text-xs text-gray-500">Regional</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-cyan-600">{operation.pointsBreakdown.communityLanguage}</div>
                      <div className="text-xs text-gray-500">Language</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-lime-600">{operation.pointsBreakdown.professionalYear}</div>
                      <div className="text-xs text-gray-500">Prof Year</div>
                    </div>
                    <div className="text-center">
