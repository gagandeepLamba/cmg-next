...
const UKBusinessOps: React.FC = () => {
  const [ukBusinessOps, setUKBusinessOps] = useState<UKBusinessOps[]>([]);
  ...
  useEffect(() => {
    const fetchUKBusinessOps = async () => {
      setLoading(true);
      try {
        const rows = await searchOperationCases({ module: 'business-uk', search: searchTerm || null, limit: 100 });
        setUKBusinessOps(rows.map((row) => mapOperationsRowToListItem(row)) as unknown as UKBusinessOps[]);
      } catch (error) {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'vat_registration': return <Badge className="bg-green-100 text-green-800">VAT Registration</Badge>;
      case 'license_application': return <Badge className="bg-indigo-100 text-indigo-800">License Application</Badge>;
  ...
  const filteredUKBusinessOps = ukBusinessOps.filter(op => {
  ...
  const stats = {
    total: ukBusinessOps.length,
    active: ukBusinessOps.filter(op => !['completed', 'on_hold'].includes(op.status)).length,
    completed: ukBusinessOps.filter(op => op.status === 'completed').length,
    highPriority: ukBusinessOps.filter(op => op.priority === 'high').length,
    totalRevenue: ukBusinessOps.reduce((sum, op) => sum + op.financials.totalCost, 0),
    pendingPayments: ukBusinessOps.filter(op => op.financials.paymentStatus === 'partial').length
  };
    ...
    <MainLayout>
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">UK Business Operations</h1>
            <p className="text-gray-600 mt-2">Manage UK business registration and operations</p>
          </div>
        ...
        <div className="space-y-4">
          {filteredUKBusinessOps.map(ukBusinessOp => (
            <Card key={ukBusinessOp.id} className="hover:shadow-md transition-shadow">
              <CardContent className="p-6">
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center space-x-3">
                    {getBusinessIcon(ukBusinessOp.businessType)}
                    <div>
                      <h3 className="text-lg font-semibold">{ukBusinessOp.companyName}</h3>
                      <p className="text-sm text-gray-600">{ukBusinessOp.leadName} • {ukBusinessOp.leadId}</p>
                    </div>
                  </div>
                  <div className="flex items-center space-x-2">
                    {getPriorityBadge(ukBusinessOp.priority)}
                    {getStatusBadge(ukBusinessOp.status)}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                  <div>
                    ...
                    <p className="font-medium">{ukBusinessOp.businessType}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-500 mb-1">Registration Number</p>
                    <p className="font-medium">{ukBusinessOp.registrationNumber}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-500 mb-1">Assigned To</p>
                    <p className="font-medium">{ukBusinessOp.assignedTo}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-500 mb-1">Next Follow-up</p>
                    <p className="font-medium">{ukBusinessOp.nextFollowUp}</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center space-x-1">
                          {getDocumentStatusIcon(ukBusinessOp.registrationStatus.companyRegistration)}
                          {getRegistrationStatusBadge(ukBusinessOp.registrationStatus.companyRegistration)}
                        </div>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-sm">Tax</span>
                        <div className="flex items-center space-x-1">
                          {getDocumentStatusIcon(ukBusinessOp.registrationStatus.taxRegistration)}
                          {getRegistrationStatusBadge(ukBusinessOp.registrationStatus.taxRegistration)}
                        </div>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-sm">VAT</span>
                        <div className="flex items-center space-x-1">
                          {getDocumentStatusIcon(ukBusinessOp.registrationStatus.vatRegistration)}
                          {getRegistrationStatusBadge(ukBusinessOp.registrationStatus.vatRegistration)}
                        </div>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-sm">Licenses</span>
                        <div className="flex items-center space-x-1">
                          {getDocumentStatusIcon(ukBusinessOp.registrationStatus.licenses)}
                          {getRegistrationStatusBadge(ukBusinessOp.registrationStatus.licenses)}
                        </div>
                      </div>
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center space-x-1">
                          {getDocumentStatusIcon(ukBusinessOp.documents.businessPlan)}
                          <span className="text-xs capitalize">{ukBusinessOp.documents.businessPlan.replace('_', ' ')}</span>
                        </div>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-sm">Financial Statements</span>
                        <div className="flex items-center space-x-1">
                          {getDocumentStatusIcon(ukBusinessOp.documents.financialStatements)}
                          <span className="text-xs capitalize">{ukBusinessOp.documents.financialStatements}</span>
                        </div>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-sm">Market Research</span>
                        <div className="flex items-center space-x-1">
                          {getDocumentStatusIcon(ukBusinessOp.documents.marketResearch)}
                          <span className="text-xs capitalize">{ukBusinessOp.documents.marketResearch.replace('_', ' ')}</span>
                        </div>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-sm">Registration Docs</span>
                        <div className="flex items-center space-x-1">
                          {getDocumentStatusIcon(ukBusinessOp.documents.registrationDocuments)}
                          <span className="text-xs capitalize">{ukBusinessOp.documents.registrationDocuments.replace('_', ' ')}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="space-y-1">
                      <div className="flex justify-between">
                        ...
                        <span className="text-sm font-medium">£{ukBusinessOp.financials.serviceFee.toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-sm">Government Fees</span>
                        <span className="text-sm font-medium">£{ukBusinessOp.financials.governmentFees.toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-sm">Total Cost</span>
                        <span className="text-sm font-medium">£{ukBusinessOp.financials.totalCost.toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-sm">Payment Status</span>
                        <Badge className={ukBusinessOp.financials.paymentStatus === 'completed' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}>
                          {ukBusinessOp.financials.paymentStatus}
                        </Badge>
                      </div>
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="space-y-2">
                      ...
                      {ukBusinessOp.milestones.every(m => m.status !== 'in_progress') && (
                ...
                <div className="flex justify-between items-center pt-4 border-t">
                  <div className="flex items-center space-x-4">
                    <div className="flex items-center space-x-1 text-sm text-gray-500">
                      ...
                      <span>Created: {ukBusinessOp.createdDate}</span>
                    </div>
                    <div className="flex items-center space-x-1 text-sm text-gray-500">
                      <Clock className="h-4 w-4" />
                      <span>Updated: {ukBusinessOp.lastUpdated}</span>
                    </div>
                  </div>
        ...
        {filteredUKBusinessOps.length === 0 && (
          <Card>
