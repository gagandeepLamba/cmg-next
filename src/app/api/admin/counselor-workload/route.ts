...
// dm_employee has no per-counselor quota column yet - this is the daily default
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const counselorRows = await sequelize.query<{
      ...
      LEFT JOIN dmc_forum_leads l ON (l.assignTo = e.id OR l.Counsilor = e.id) AND ${leadPeriodClause}
    ...
    const [unassignedRow] = await sequelize.query<{ total: number }>(
      `SELECT COUNT(*) AS total FROM dmc_forum_leads l
