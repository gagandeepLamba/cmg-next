/**
 * Syncs Meta Ad campaigns into dm_meta_campaign_cache.
 * Shared by the /api/cron/meta-campaign-sync route and the in-process scheduler.
 */

import { QueryTypes } from 'sequelize';
import { sequelize } from '@/lib/sequelize';
import { fetchCampaigns } from './graph-api';

export type CampaignSyncResult =
  | { skipped: true; reason: string }
  | { skipped: false; synced: number };

export async function syncCampaigns(): Promise<CampaignSyncResult> {
  ...
  if (!settings?.is_enabled || !settings?.ad_account_id) {
    return { skipped: true, reason: 'Integration disabled or ad account not configured' };
  }
  ...
  return { skipped: false, synced: campaigns.length };
}
