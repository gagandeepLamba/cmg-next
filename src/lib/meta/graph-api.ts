...
import { getActiveAccessToken } from './token-manager';
...
export async function fetchLeadFromMeta(leadgenId: string): Promise<MetaRawLead> {
  ...
  const url = `${BASE}/${apiVersion()}/${leadgenId}?fields=${fields}&access_token=${token}`;
...
export async function fetchFormName(formId: string): Promise<string | null> {
  try {
    const token = await getActiveAccessToken();
    const url = `${BASE}/${apiVersion()}/${formId}?fields=name&access_token=${token}`;
