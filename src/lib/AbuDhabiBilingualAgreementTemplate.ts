// Bilingual (EN/AR) "Agreement for Advisory Services" — Abu Dhabi branch
// (Didactic Management Consultants - L.L.C - S.P.C).
//
// This is a SEPARATE template from bilingualAgreementTemplate.ts (the 4-branch
// Gulf template for Abu-Dhabi-style DED-licensed branches whose clauses are
// byte-identical) because Didactic's signed PDF is NOT byte-identical to that
// shared body:
//   - Clause 7 carries an extra "7.5 DURATION AND RENEWAL" sub-clause (a flat,
//     non-refundable USD 500 Agreement Renewal Fee per renewal term) that the
//     shared template does not have.
//   - Fee Summary carries a matching "AGREEMENT RENEWAL FEE" line.
//   - Clause 14 is "GOVERNING LAW AND DISPUTE RESOLUTION" — DIAC arbitration
//     seated in Dubai — not the shared template's court-jurisdiction clause.
//   - Annexure A carries an extra "Program Term Schedule reference (6/12/18
//     months)" field, and the top Agreement Details box carries a matching
//     "Program Code / Service Term (see Program Term Schedule)" field.
// Everything else (structure, styling, helper functions, field layout) mirrors
// bilingualAgreementTemplate.ts on purpose, so the two stay easy to diff.
...
export interface AbuDhabiAgreementValues {
...
const COMPANY = {
  ...
  addressAr: '1802، مبنى سالم HQ، شارع السالم، أبوظبي، الإمارات العربية المتحدة',
  regulatoryEn: 'Commercial Licence No. CN-2539189 - Abu Dhabi Department of Economic Development; licensed activities: Visa Services and Administrative Consultancy & Studies',
  regulatoryAr: 'الرخصة التجارية رقم CN-2539189 - دائرة التنمية الاقتصادية أبوظبي؛ الأنشطة المرخصة: خدمات استخراج التأشيرات والاستشارات والدراسات الإدارية',
...
const getAgreementDetails = (v: AbuDhabiAgreementValues) => {
