// Bilingual (EN/AR) "Agreement for Advisory Services" — Qatar branch
// (Disha Management Consultants LLC).
//
// Sibling file to abuDhabiAgreementTemplate.ts, dubaiAgreementTemplate.ts and
// kuwaitAgreementTemplate.ts. Kept as a separate module for the same reason
// as those: this branch's signed PDF is not byte-identical to the shared
// Gulf template body. Like Kuwait (and unlike Abu Dhabi/Dubai), Qatar's
// Clause 14 is plain COURT JURISDICTION, not DIAC arbitration, and the
// Agreement Details box has no "Agreement Expiry" field. Two further
// branch-specific points worth flagging:
//   - The jurisdiction clause names Doha specifically ("competent courts of
//     Doha, Qatar"), not just "courts of Qatar" the way Kuwait's names the
//     state generally — copied verbatim from the signed PDF.
//   - The client ID field here is "Passport/QID No." (Qatar ID), and the
//     signed PDF also carries the company's phone/email inline in the
//     Service-Provider address line — both reproduced verbatim below.
...
// clause bodies) matches the established pattern from the other three
// branch files, with Qatar's branch identity and QAR currency substituted
// in.
...
export interface QatarAgreementValues {
...
const COMPANY = {
  nameEn: 'Disha Management Consultants LLC',
  nameAr: 'شركة ديشا للاستشارات الإدارية ذ.م.م',
  addressEn: 'Abdul Jaleel Business Center, Office 301, 3rd Floor, Office Block, Airport Street, next to Ford Showroom, Doha, Qatar. Phone: +974 4436 7929 | Email: info@dm-consultant.qa',
  addressAr: 'مركز عبد الجليل للأعمال، مكتب 301، الطابق الثالث، مبنى المكاتب، شارع المطار، بجوار معرض فورد، الدوحة، قطر. الهاتف: 974 4436 7929+ | البريد الإلكتروني: info@dm-consultant.qa',
  regulatoryEn: 'Commercial Registration No. 133895 - State of Qatar',
  regulatoryAr: 'السجل التجاري رقم 133895 - دولة قطر',
...
const clauses: Array<[string, string, string, string]> = [
  ...
  // arbitration clause, and names Doha specifically as the seat of
  // jurisdiction (unlike Kuwait's more general "courts of Kuwait").
  ['14. GOVERNING LAW AND JURISDICTION',
    'This Agreement is governed by the laws of the State of Qatar. Subject to mandatory consumer or regulatory jurisdiction, the parties submit to the exclusive jurisdiction of the competent courts of Doha, Qatar. The English and Arabic texts are intended to correspond; if an inconsistency cannot be reconciled, the language given priority by mandatory local law shall prevail, otherwise the English text shall be used for contractual interpretation.',
    '14. القانون واجب التطبيق والاختصاص',
    'تخضع هذه الاتفاقية لقوانين دولة قطر. ومع مراعاة أي اختصاص إلزامي للمستهلك أو الجهات التنظيمية، ينعقد الاختصاص الحصري لمحاكم الدوحة المختصة، دولة قطر. يقصد أن يتطابق النصان الإنجليزي والعربي؛ وإذا تعذر التوفيق بينهما، تسود اللغة التي يمنحها القانون المحلي الإلزامي الأولوية، وإلا فيستخدم النص الإنجليزي لتفسير العقد.'],
...
const getAgreementDetails = (v: QatarAgreementValues) => {
...
export function renderQatarAgreement(v: QatarAgreementValues): string {
...
رسوم الحكومة / الجهات / الأطراف الثالثة: غير مشمولة ما لم ينص الملحق أ على خلاف ذلك.</p>
        ...
        <p class="renewal-fee">رسوم تجديد الاتفاقية: تُستحق رسوم ثابتة وغير قابلة للاسترداد قدرها ${COMPANY.renewalFeeUsdAr} (أو ما يعادلها بالريال القطري) عن كل مدة تجديد، بصرف النظر عن مدة البرنامج — انظر البند 7.5.</p>
      </div>
    </section>
    </div>
